...
void main() {
    ...
    // newFakeUv = fract(newFakeUv + vec2(uTime/1000.0, uTime/1000.0));
    ...
    vec2 modifiedUv = fract(vUv + vec2(uTime/30.0, uTime/20.0));
    vec4 texture = texture2D(graffitiTexture, modifiedUv + 0.1 * cnoise(vec4(vUv*25.0, uTime/10.0, 0.0), vec4(5.0)));
