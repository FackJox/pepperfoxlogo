...
import uniforms from "./MSDFTextMaterial/uniforms";
...
export function PepperfoxMSDF1(props) {
	...
	// });
	const { masterProgress } = { ... props }

	const { lightPos } = { ...props };

	// const { progress1, progress2, progress3, progress0, masterProgress } = useControls({
	...
	// 	masterProgress: { value: 0.0, min: 0.0, max: 1.5 },
	// });
 ...
 const progressTest = 0.2
		...
		const material = new THREE.ShaderMaterial({
			...
			uniforms: {
				...
				...{
					...
					uMasterProgress: { value: masterProgress },
    ...
    `,
        ...
        void main() {
			...
			vec3 red = vec3(0.898,0.137,0.133);
			vec3 orange = vec3(0.922,0.357,0.129);
			vec3 purple = vec3(0.282,0.153,0.286);
			vec3 pink = vec3(0.918,0.569,0.741);
			vec3 green = vec3(0.729,0.847,0.698);

            // Outputs
            #include <three_msdf_strokes_output>

			vec4 l1 = vec4(green, border);
			vec4 l2 = vec4(pink, border);
			vec4 l3 = vec4(pink, outset);
			vec4 l4 = vec4(purple, outset);
			vec4 l5 = vec4(purple, outset);
			vec4 l6 = vec4(red, outset);
			vec4 l7 = vec4(orange, outset);
			...
			float uProgress0 = uMasterProgress - 0.05;
			float uProgress1 = uMasterProgress - 0.2;
			float uProgress2 = uMasterProgress - 0.3;
			float uProgress3 = uMasterProgress - 0.4;
			float uProgress4 = uMasterProgress - 0.3;
			float uProgress5 = uMasterProgress - 0.5;
			float uProgress6 = uMasterProgress - 0.6;
			...
			mix4 = clamp(mix4, 0., 1.0 );

									
			float p5 = uProgress5;
			p5 = map(p5, 0., 1., -w, 1.);
			p5 = smoothstep(p5,p5+w, vLayoutUv.x);
			float mix5 = 2.* p5 - pattern;
			mix5 = clamp(mix5, 0., 1.0 );

									
			float p6 = uProgress6;
			p6 = map(p6, 0., 1., -w, 1.);
			p6 = smoothstep(p6,p6+w, vLayoutUv.x);
			float mix6 = 2.* p6 - pattern;
			mix6 = clamp(mix6, 0., 1.0 );
			...
			vec4 layer4 = mix(layer3, l5, 1.-mix4);
			vec4 layer5 = mix(layer4, l6, 1.-mix5);
			vec4 layer6 = mix(layer5, l7, 1.-mix6);
    ...
    `,
		...
		pepperfoxMSDFRef1.current.geometry = geometry;
		pepperfoxMSDFRef1.current.material = material;
		pepperfoxMSDFRef1.current.scale.set(0.009, 0.009, 0.009);
		pepperfoxMSDFRef1.current.position.set(-1.0, 0, 0);
		pepperfoxMSDFRef1.current.rotation.set(
			0,
			(180 * Math.PI) / 180,
			(180 * Math.PI) / 180
		);
